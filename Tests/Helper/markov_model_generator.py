...
class MarkovModelGenerator:
    def __init__(self, transition_matrix: np.typing.NDArray, init_state: int):
        ...
        self._n_states = len(transition_matrix)
        self._mode_centers: np.typing.NDArray = np.linspace(0, self._n_states, self._n_states)
    ...
    def get_next(self, batch_size: int, sequence_length: int, dimension, rng: Generator) -> np.typing.NDArray:
        ...
        for _ in range(batch_size):
            ...
            for _ in range(sequence_length):
                measurement: np.typing.NDArray = rng.standard_normal(dimension) + self._mode_centers[self._state]
    ...
    def _next_state(self, rng: Generator):
        ...
        self._state = rng.choice(self._n_states, p=transition_probabilities/np.sum(transition_probabilities))
